...
function Inbox() {
  const [mailSelectedType, setMailSelectedType] = useState(0);
          ...
          {mailType.map((item, index) => {
            return (
              <button
                key={index}
                className={`${
                  mailSelectedType === index
                    ? "border-b-4 border-b-blue-600 text-blue-600"
                    : "border-b-4 border-b-transparent"
                } flex items-center gap-5 p-4 w-52 hover:bg-gray-300`}
                onClick={() => setMailSelectedType(index)}
              >
                {item.icon}
                <span>{item.text}</span>
              </button>
